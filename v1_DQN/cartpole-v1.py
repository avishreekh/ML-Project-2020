...
> GOAL: minimize squared error between the prediction and target
'''
...
def nn_model(inp_shape, action_space):
    ...
    X = Dense(512, input_shape=inp_shape, activation='relu', kernel_initializer='he_uniform')(X)
    X = Dense(128, input_shape=inp_shape, activation='relu', kernel_initializer='he_uniform')(X)
    ...
    def execute(self):
        for ep in range(self.episodes):
            ...
            while not done:
                ...
                iter += 1
                if done:
                    print("episode: {}/{}, score: {}, ep: {:.2}".format(ep, self.episodes, iter, self.epsilon))
                    if iter >= 500:
